...
def fix_schema():
    ...
    if 'trial_ends_at' not in columns: add_col('company', 'trial_ends_at', 'DATETIME')
    if 'trial_start_date' not in columns: add_col('company', 'trial_start_date', 'DATETIME')
    if 'trial_end_date' not in columns: add_col('company', 'trial_end_date', 'DATETIME')
    if 'next_due_date' not in columns: add_col('company', 'next_due_date', 'DATE')
    if 'last_active_at' not in columns: add_col('company', 'last_active_at', 'DATETIME')
    if 'updated_at' not in columns: add_col('company', 'updated_at', 'DATETIME')
    if 'plan_id' not in columns: add_col('company', 'plan_id', 'VARCHAR(50)')
    if 'last_payment_at' not in columns: add_col('company', 'last_payment_at', 'DATETIME')
